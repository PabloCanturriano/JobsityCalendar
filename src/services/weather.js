...
export function getCityInfo(city, date){
    const params = `/${city.replace("","%20")}/${date}/${date}?unitGroup=metric&key=32AA83DX7L9K39JLU4KVD6234&contentType=json`;
    return fetch(apiUrl + params, {mode: 'cors'})
        .then((res) => res.json())
        .then((json) => {
            const data = json.days[0];
            return {
